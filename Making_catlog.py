...
path2 = r"neighbor_counts_edr3_all.npz"
tmp = np.load(path2) # 30749278 pairs 
...
def query_this_j(j):
    ...
    for i, idxs in enumerate(these_inds):
        ...
        brighter_parallax[G[idxs] > G_block[i]] = parallax_block[i] # parallax of the brighter component For each potential companion: If the companion's G magnitude (G[idxs]) is greater than the current star's G 
                                                                    # magnitude (G_block[i]): The current star is brighter than the companion. The parallax of the brighter star is assigned to be the parallax of the 
                                                                    # current star (parallax_block[i]).
...
star1s = np.concatenate([result[0] for result in all_result])
star2s = np.concatenate([result[1] for result in all_result])
